...
def test_fetch_connection_status(monkeypatch):
    def fake_fetch(url: str, timeout: float, *, user_agent=None):
...
def test_fetch_connection_status_with_user_agent(monkeypatch):
    ...
    status = asyncio.run(
        fetch_connection_status("https://example.com/status", user_agent="UA/1.0")
    )
    assert status.active_connections is None
    assert status.max_connections is None
